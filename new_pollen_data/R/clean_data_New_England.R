...
saveRDS(pollen.counts.cleaned,paste(data.loc,'additional_pollen_New_England_clean.RDS',sep=''))
...
saveRDS(depths,paste(data.loc,'additional_pollen_New_England_depths.RDS',sep=''))


site_names <- names(depths)
site_names <- matrix(unlist(strsplit(site_names,'_')),ncol=2,byrow=TRUE)[,1]
saveRDS(site_names,paste(data.loc,'additional_pollen_New_England_site_names.RDS',sep=''))
