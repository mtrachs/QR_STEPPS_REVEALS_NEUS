...
setwd('~/workflow_stepps_calibration/')
data.loc <- 'new_pollen_data/'
help.fun.loc <- 'calibration/calibration_helper_funs/' 
...
calib_trans <- 
  sapply(1:length(lake.names),function(zz){
        ...
        analogue::join(agg.daten,fake.counts)$agg.daten       
  })

additional_pollen <- t(calib_trans)


saveRDS(additional_pollen,paste(save.loc,'data_paciorek_mclachlan.RDS'))
