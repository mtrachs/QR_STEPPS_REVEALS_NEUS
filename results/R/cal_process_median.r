...
# 
# 
# file.list <- list.files(paste(path_data,'/stepps_median/',sep=''))
# ind.file <- grep(file.name,file.list)
# nr1 <- unlist(strsplit(file.list[ind.file],'[.]'))[seq(1,2*length(ind.file),2)]
# if(suff_fit == 'cal_pl_NEUS') nr1 <- nr1[nchar(nr1) < 16]
# nr.available <- as.numeric(substr(nr1,nchar(nr1),nchar(nr1)))
# nr.available <- nr.available[!is.na(nr.available)]
# 
# for (i in nr.available) {
#   fname = paste(path_data,'stepps_median/',file.name,i,'.csv',sep='') #sprintf('%s/%s.csv', path_out, suff_fit)
#   fit <- read_stan_csv(fname)
# #fit1 <- read_stan_csv(fname1)
#   if (i==min(nr.available))  {post = rstan::extract(fit, permuted=FALSE, inc_warmup=FALSE)}
...
#dispersal_decay(post, dmat, sum_w, radius, run, taxa)
 ...
 seventy  = apply(r_int,2,function(x) 1000*radius[which.min(abs(x - 0.7))])
