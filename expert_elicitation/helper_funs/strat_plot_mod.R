strat.plot.mod <- 
function (d, yvar = NULL, scale.percent = FALSE, graph.widths = 1, 
          ...
          y.rev = FALSE, y.tks = NULL, ylabel = "", cex.ylabel = 1, 
          cex.yaxis = 0.8, xSpace = 0.01, x.pc.inc = 10, x.pc.lab = TRUE, 
          x.pc.omit0 = TRUE, wa.order = "none", plot.line = TRUE, col.line = "black", 
          lwd.line = 1, plot.bar = TRUE, lwd.bar = 1, col.bar = "grey", 
          ...
          mgp = NULL, cex.axis = 0.8, clust = NULL, clust.width = 0.1, 
          ...
          ...) 
{
  ...
  else cc.bar <- rep(col.bar,1)#, length.out = nsp)
  ...
  if (wa.order == "topleft" || wa.order == "bottomleft") {
    colsum <- colSums(d)
    opt <- (t(d) %*% yvar)/colsum
    if ((wa.order == "topleft" & !y.rev) | (wa.order == "bottomleft" & 
                                            y.rev)) 
  ...
  if (y.axis) {
    if (is.null(y.tks)) 
      y.tks <- axTicks(2)
    ax <- axis(side = 2, las = 1, at = y.tks, labels = as.character(y.tks), 
    ...
    if (scale.percent) {
      ...
      if (bar.back) {
        if (is.logical(plot.bar)) {
          if (plot.bar) {
            #cc.bar[seq(5,nrow(d),5)] <- 'red'
            if (sep.bar) {
              segments(rep(0, nsam), yvar, d[, i], yvar, 
      ...
      if (plot.symb) {
        points(d[, i], yvar, pch = symb.pch, cex = symb.cex, 
               xpd = NA)
      }
      if (plot.poly) {
        y <- c(min(yvar, na.rm = TRUE), yvar, max(yvar, 
                                                  na.rm = TRUE))
        x <- c(0, d[, i], 0)
        if (exag[i]) {
      ...
      lines(c(0, 0), c(min(yvar, na.rm = TRUE), max(yvar, 
                                                    na.rm = TRUE)), ...)
      if (ty == "l") 
      ...
      if (!is.null(minmax)) {
        plot(d[, i], yvar, cex = 0.5, axes = FALSE, xaxs = "i", 
             type = "n", yaxs = "r", ylim = ylim, xlim = c(minmax[i, 
                                                                  1], minmax[i, 2]), ...)
      }
      else {
        plot(d[, i], yvar, cex = 0.5, axes = FALSE, xaxs = "i", 
      ...
      if (bar.back) {
        if (is.logical(plot.bar)) {
          if (plot.bar) {
            if (sep.bar) {
              segments(rep(us[1], nsam), yvar, d[, i], 
                       yvar, lwd = lwd.bar, col = cc.bar)
            }
            else {
              segments(rep(us[1], nsam), yvar, d[, i], 
                       yvar, lwd = lwd.bar, col = cc.bar)
      ...
      if (!bar.back) {
        if (is.logical(plot.bar)) {
          if (plot.bar) {
            if (sep.bar) {
              cc.bar[seq(5,nrow(yvar),5)] <- 'red'
              segments(rep(us[1], nsam), yvar, d[, i], 
                       yvar, lwd = lwd.bar, col = cc.bar )#cc.bar)
            }
            else {
              cc.bar[seq(5,nrow(yvar),5)] <- 'red'
              segments(rep(us[1], nsam), yvar, d[, i], 
                       yvar, lwd = lwd.bar, col = cc.bar)#cc.bar[i])
            }
          }
        }
        else {
      ...
      lines(c(us[1], us[1]), c(min(yvar, na.rm = TRUE), 
                               max(yvar, na.rm = TRUE)), ...)
      if (ty == "l") 
        lines(d[, i], yvar, col = cc.line[i], lwd = lwd.line)
      if (!is.null(fun2[i])) {
  ...
  if (!is.null(clust)) {
    ...
    plot(clust, xvar = yvar, horiz = TRUE, x.rev = y.rev, 
         labels = rep("", length(yvar)), hang = -1, mgp = mgpX, 
         cex.axis = cex.axis, ...)
  }
  par(mai = oldmai)
  oldfig[oldfig < 0] <- 0
