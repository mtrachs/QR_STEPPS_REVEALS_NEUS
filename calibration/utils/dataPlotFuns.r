...
pieMap=function(proportions, 
                ...
                cex.legend=1.2,
                pt.cex=1.6,
                state.plot.index=3){#c("#1F78B4", "#33A02C", "#E31A1C", "#6A3D9A", "#B15928", "#FF7F00", 
  ...
  plot(us.shp[state.plot.index,], add=TRUE, lwd=2)
  n=length(centers[,1])
...
#   }
  ...
  if (add_legend){
    ...
    #     900000,1500000
    legend('topleft', colnames(proportions), pch=rep(22,n), pt.cex=pt.cex, cex=cex.legend, pt.bg=cols, col=rep('black', n),
           bg='white', ncol=2)
