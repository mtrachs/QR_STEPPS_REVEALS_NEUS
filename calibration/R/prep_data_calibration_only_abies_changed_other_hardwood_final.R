...
if('veg_mean.RDS'%in%list.files(data.loc)){
  ...
  source(paste(help.fun.loc,'load_vegetation_data_median.R',sep='')) # have to download a large file manually to execute this script
  veg_mean <- total.dat
...
#extract vegetation data that is in the bounding box
  ...
  veg_mean <- replace(veg_mean,veg_mean==0,2e-7) #does this have an effect there shouldn't be any zero... exp(-Inf) = 0
  reconst_grid <- build_grid(veg_box, resolution = 8000, proj = '+init=epsg:3175')
...
source(paste0(wd,'expert_elicitation/helper_funs/get_meta_data.R'))
...
########################################################################################################################
#load samples from two additional lakes
########################################################################################################################
...
#################################################################################################################
write.csv(sort(colnames(pollen.test)),paste0(wd,'calibration/data/taxon_names.csv'))
  
####################################################################################################################
# save lake names
####################################################################################################################
lake.names <- pollen.test$site.name
site.ids <- pollen.test$site.id
saveRDS(lake.names,paste(data.loc,'lake_names.RDS',sep=''))
saveRDS(site.ids,paste(data.loc,'site_ids.RDS',sep=''))
#####################################################################################################################
...
###############################################################################################################
# additional_pollen <- readRDS(paste(data.loc,'data_paciorek_mclachlan.RDS',sep=''))
# col_nam <- colnames(additional_pollen)
# additional_pollen <- matrix(ncol=length(col_nam),as.numeric(unlist(additional_pollen)))
# additional_pollen <- as.data.frame(additional_pollen)
# colnames(additional_pollen) <- col_nam 
# 
# calib_trans <- analogue::join(calib_trans,additional_pollen,split = FALSE)
# col_nam1 <- colnames(calib_trans)
# calib_trans <- matrix(ncol=ncol(calib_trans),unlist(calib_trans))
# colnames(calib_trans) <- col_nam1
...
library(rstan)
stan_rdump(list = names(test_neus), file = paste(data.loc,'/elicitation_neus_certainty_median_',num_sites,'_sites_only_abies_new_species.dump',sep=''))

veg_coords <- veg_table@coords
pollen_coords <- pol_table@coords
save(list = c(names(test_neus),'veg_coords','pollen_coords'), 
     file = paste(data.loc,'elicitation_neus_certainty_median_',num_sites,'_sites_only_abies_new_species.RData',sep=''))


# #-------------------------------------------------------------------------------------------------------------------
#also save data as .csv
write.csv(calib_trans,paste(data.loc,'pollen_elicitation_NEUS.csv',sep=''))
write.csv(veg_mean,paste(data.loc,'vegetation_mean_NEUS.csv',sep=''))
